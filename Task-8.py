...
class Expense(User):
    ...
    def highest_expense(self):
        ...
        cursor.execute("Select category,amount from expenses where user_id = %s",[self.user_id])
        exp_list = cursor.fetchall()
        food_list = [i[1] for i in exp_list if i[0] == "Food"]
        travel_list = [i[1] for i in exp_list if i[0] == "Travel"]
        shopping_list = [i[1] for i in exp_list if i[0] == "Shopping"]

        food_exp = functools.reduce(lambda a,b: a+b, food_list,0)
        travel_exp = functools.reduce(lambda a,b: a+b, travel_list,0)
        shopping_exp = functools.reduce(lambda a,b: a+b, shopping_list,0)  
